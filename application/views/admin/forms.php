...
<div
	...
	<div class="green-box">
		<h3>
		<?php echo Kohana::lang('ui_main.form_has_been');?>
	...
	<div class="tabs">
		...
		<?php print form::open(NULL,array('id' => 'formMain',
			...
			<input type="hidden" id="form_id" name="form_id"
				value="<?php echo $form['form_id']; ?>" /> <input type="hidden"
				id="form_active" name="form_active" value="" /> <input type="hidden"
			...
			<table class="table">
				...
				foreach ($forms as $form)
				{
					...
					<input type="hidden" name="action"
						id="action_<?php echo $form_id;?>" value="">
					...
					<tr id="tr_<?php echo $form_id; ?>">
						...
						<td class="col-2">
							<div class="post">
								...
								<?php echo $form_title; ?>
									&nbsp;&nbsp;&nbsp;[<a
										href="javascript:showForm('formDiv_<?php echo $form_id; ?>')"><?php echo Kohana::lang('ui_main.edit_form_fields');?>
									</a>]
								</h4>
								<p>
						...
						<td class="col-4">
							<ul>
								<li class="none-separator"><a href="#add"
									onClick="fillFields('<?php echo(rawurlencode($form_id)); ?>','<?php echo(rawurlencode($form_title)); ?>','<?php echo(rawurlencode($form_description)); ?>')">Edit</a>
								</li>
								<li class="none-separator"><a
									href="javascript:formAction('h','SHOW/HIDE','<?php echo(rawurlencode($form_id)); ?>')"
									<?php if ($form_active) echo " class=\"status_yes\"" ?>><?php echo Kohana::lang('ui_main.active');?>
								</a></li>
								<li><a
									href="javascript:formAction('d','DELETE','<?php echo(rawurlencode($form_id)); ?>')"
					...
					<tr style="margin: 0; padding: 0; border-width: 0;">
						<td colspan="4" style="margin: 0; padding: 0; border-width: 0;">
							<div id="formDiv_<?php echo $form_id; ?>" class="forms_fields">
								<a href="javascript:addNewForm('<?php echo $form_id; ?>')"
									class="new-form_field">Add New Field</a>
								<div id="formadd_<?php echo $form_id; ?>"
									class="forms_fields_add">
									<div class="tab">
										<div>
										<?php echo form::open(url::site() . 'admin/manage/forms/field_add', array('id' => 'form_field_'.$form_id,
											...
											<div id="form_fields_<?php echo $form_id; ?>"
												class="forms_fields_new"></div>
												<?php echo form::close(); ?>
										</div>
									</div>
								</div>
								<div id="form_fields_current_<?php echo $form_id; ?>"
									class="forms_fields_current">
