...
<div
	...
	<div class="tabs">
		...
		<?php print form::open(NULL,array('enctype' => 'multipart/form-data',
			...
			<input type="hidden" id="category_id" name="category_id"
				value="<?php echo $form['category_id']; ?>" /> <input type="hidden"
			...
			<div class="tab_form_item">
				...
				<br /> <a href="#" id="category_translations" class="new-cat"
					style="clear: both;">Category Translations</a>
	...
	<?php print form::open(NULL,array('id' => 'catListing',
					 	'name' => 'catListing')); ?>
		<input type="hidden" name="action" id="category_action" value=""> <input
			type="hidden" name="category_id" id="category_id_action" value="">
			...
			<table class="table" id="categorySort">
				...
				foreach ($categories as $category)
				{
					...
					<tr id="<?php echo $category_id; ?>">
						...
						<td class="col-3"><?php if (!empty($category_image))
						{
						...
						<td class="col-4">
							<ul>
								<li class="none-separator"><a href="#add"
									onClick="fillFields('<?php echo(rawurlencode($category_id)); ?>','<?php echo(rawurlencode($parent_id)); ?>','<?php echo(rawurlencode($category_title)); ?>','<?php echo(rawurlencode($category_description)); ?>','<?php echo(rawurlencode($category_color)); ?>','<?php echo(rawurlencode($category_image)); ?>'<?php
													foreach($locale_array as $lang_key => $lang_name){
														echo ',';
								...
								<li class="none-separator"><a class="status_yes"
									href="javascript:catAction('v','SHOW/HIDE','<?php echo(rawurlencode($category_id)); ?>')"><?php if ($category_visible) { echo Kohana::lang('ui_main.visible'); } else { echo Kohana::lang('ui_main.hidden'); }?>
								</a></li>
								<li><a
									href="javascript:catAction('d','DELETE','<?php echo(rawurlencode($category_id)); ?>')"
									class="del"><?php echo Kohana::lang('ui_main.delete');?> </a></li>
							</ul> <?php if($category_trusted == 1) { ?>
							<div class="right">
								<a href="#" class="tooltip"
									title="<?php echo htmlentities(Kohana::lang('ui_admin.special_category_explanation'),ENT_QUOTES);?>"><strong><?php echo Kohana::lang('ui_admin.special_category');?>
								</strong> </a>
							</div> <?php } ?>
						</td>
					</tr>
						...
						<td class="col-4">
							<ul>
								...
								<li class="none-separator"><a
									href="javascript:catAction('v','SHOW/HIDE','<?php echo(rawurlencode($category_id)); ?>')"
									<?php if ($category_visible) echo " class=\"status_yes\"" ?>><?php echo Kohana::lang('ui_main.visible');?>
								</a></li>
								<li><a
									href="javascript:catAction('d','DELETE','<?php echo(rawurlencode($category_id)); ?>')"
									class="del"><?php echo Kohana::lang('ui_main.delete');?> </a></li>
