			...
			<form method="POST" name="frm_install" action="process.php"
				...
				<table class="form-table fields">
					<tbody>
						<tr>
							...
							<td><input type="text"
								value="<?php print $form->value('base_path') == "" ? $install->_get_base_path($_SERVER["REQUEST_URI"]) : $form->value('base_path'); ?>"
								size="25" id="base_path" name="base_path" /></td>
							<td>The location on your server where you placed your Ushahidi
								files. <strong>We have automatically detected this value, please
									make sure that it is correct.</strong> If the field is empty,
								do not worry, it means ushahidi is installed at the top level
								directory.</td>
						</tr>
						<tr>
							<th scope="row"><label for="db_name">Database Name</label></th>
							<td><input type="text"
								value="<?php print $form->value('db_name') == "" ? !empty($_SESSION['db_name']) : $form->value('db_name'); ?>"
								size="25" id="db_name" name="db_name" /></td>
							<td>The name of the database you want to run Ushahidi in.</td>
						</tr>
						<tr>
							<th scope="row"><label for="username">User Name</label></th>
							<td><input type="text"
								value="<?php print $form->value('username') == "" ? !empty($_SESSION['username']) : $form->value('username'); ?>"
								size="25" id="username" name="username" /></td>
							<td>Your database username.</td>
						</tr>
						<tr>
							<th scope="row"><label for="pwd">Password</label></th>
							<td><input type="password"
								value="<?php print $form->value('password') == "" ? !empty($_SESSION['password']) : $form->value('password'); ?>"
								size="25" id="password" name="password" /></td>
						...
						<tr>
							...
							<td><input type="text"
								...
								size="25" id="host" name="host" /></td>
							<td>If you are running Ushahidi on your own computer, this will
								more than likely be "localhost". If you are running Ushahidi
								from a web server, you'll get your host information from your
								web hosting provider.</td>
						</tr>
						<tr>
							<th scope="row"><label for="table_prefix">Table Prefix</label></th>
							<td><input type="text" size="25"
								value="<?php print $form->value('table_prefix') == "" ? !empty($_SESSION['table_prefix']) : $form->value('table_prefix'); ?>"
								id="table_prefix" name="table_prefix" /></td>
							<td>Normally you won't change the table prefix. However, If you
								want to run multiple Ushahidi installations from a single
								database you can do that by changing the prefix here.</td>
						</tr>
				...
				<table class="form-table">
					<tbody>
						<tr>
							<td class="next">
								<!--<input type="button" class="button" value="&larr; Previous" />-->
							</td>
							<td class="prev">
								<!--<input type="button" class="button" value="Continue &rarr;" value="submit"	 />-->
								<input type="submit" id="install" name="advanced_db_info"
								value="Continue &rarr;" class="button" /></td>
						</tr>
					</tbody>
				</table>
			</form>
